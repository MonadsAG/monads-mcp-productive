...
const updateTaskStatusSchema = z.object({
  ...
  status_name: z
    .string()
    .optional()
    .describe('Status name to match, e.g. "In Progress", "Done", "QA Review"'),
});
...
export const updateTaskStatusDefinition = {
  ...
  description:
    'Update the status of a task in Productive.io. You can provide a status_name (e.g. "In Progress", "Done", "QA Review") and it will automatically resolve to the correct workflow status for that task. Supports custom workflow statuses. Falls back to workflow_status_id if provided directly.',
  inputSchema: {
    type: 'object',
    properties: {
      task_id: {
      ...
      status_name: {
        type: 'string',
        description:
          'Name of the status to set (e.g. "In Progress", "Done", "To Do"). Case-insensitive, supports partial matching. If ambiguous, returns available options.',
      },
      workflow_status_id: {
        type: 'string',
        description:
          'Exact workflow status ID (optional — use status_name instead for convenience)',
      },
    },
    required: ['task_id'],
  },
